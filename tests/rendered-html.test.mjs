...
test("daily purchase drafts generate all fixed traceable ticket types and settle alternatives",()=>{
 ...
 assert.equal(set.plans.length,15);assert.deepEqual(set.plans.map(plan=>plan.id),["score-double-3","score-single-2","score-double-2","score-single-3","total-double-3","total-double-2","total-single-2","draw-or-handicap-draw-2","draw-or-handicap-draw-3","result-mixed-3","result-mixed-4","result-mixed-5","had-safe-2","total-adjacent-double-2","half-full-double-3"]);
 ...
 assert.equal(set.plans[14].passName,"3串1");assert.equal(set.plans[14].betCount,8);assert.equal(set.plans[14].stake,16);assert.ok(set.plans[14].minWinningProfit>0);
